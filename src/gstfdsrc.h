...
#define GST_TYPE_FD_SRC2 \
  (gst_fd_src_get_type())
#define GST_FD_SRC(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj),GST_TYPE_FD_SRC2,GstFd2Src))
#define GST_FD_SRC_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST((klass),GST_TYPE_FD_SRC2,GstFd2SrcClass))
#define GST_IS_FD_SRC(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj),GST_TYPE_FD_SRC2))
#define GST_IS_FD_SRC_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_TYPE((klass),GST_TYPE_FD_SRC2))


typedef struct _GstFd2Src GstFd2Src;
typedef struct _GstFd2SrcClass GstFd2SrcClass;

/**
 * GstFd2Src:
 *
 * Opaque #GstFd2Src data structure.
 */
struct _GstFd2Src {
  GstPushSrc element;
