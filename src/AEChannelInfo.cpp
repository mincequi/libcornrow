...
AudioChannelLayout::AudioChannelLayout(const enum AudioChannel* rhs)
{
    *this = rhs;
}

AudioChannelLayout::AudioChannelLayout(const AEStdChLayout rhs)
{
    *this = rhs;
}

void AudioChannelLayout::ResolveChannels(const AudioChannelLayout& rhs)
{
...
AudioChannelLayout& AudioChannelLayout::operator=(const AudioChannelLayout& rhs)
{
    if (this == &rhs)
        return *this;
    ...
    memcpy(m_channels, rhs.m_channels, sizeof(enum AudioChannel) * rhs.m_channelCount);

    return *this;
}

AudioChannelLayout& AudioChannelLayout::operator=(const enum AudioChannel* rhs)
{
    clear();
    if (rhs == NULL)
        return *this;
...
AudioChannelLayout& AudioChannelLayout::operator=(const enum AEStdChLayout rhs)
{
    assert(rhs > AE_CH_LAYOUT_INVALID && rhs < AE_CH_LAYOUT_MAX);

    static enum AudioChannel layouts[AE_CH_LAYOUT_MAX][9] = {
    {AE_CH_FC, AE_CH_NULL},
...
bool AudioChannelLayout::operator==(const AudioChannelLayout& rhs) const
{
...
AudioChannelLayout& AudioChannelLayout::operator+=(AudioChannel rhs)
{
...
AudioChannel AudioChannelLayout::operator[](unsigned int i) const
{
    assert(i < m_channelCount);
    return m_channels[i];
}

AudioChannelLayout::operator std::string() const
{
    if (m_channelCount == 0)
        return "NULL";
...
bool AudioChannelLayout::IsChannelValid(const unsigned int pos)
{
...
bool AudioChannelLayout::IsLayoutValid()
{
    if (m_channelCount == 0)
        return false;
...
const char* AudioChannelLayout::GetChName(const enum AudioChannel ch)
{
...
bool AudioChannelLayout::HasChannel(const enum AudioChannel ch) const
{
    for (unsigned int i = 0; i < m_channelCount; ++i)
        if (m_channels[i] == ch)
            return true;
...
bool AudioChannelLayout::ContainsChannels(const AudioChannelLayout& rhs) const
{
...
void AudioChannelLayout::ReplaceChannel(const enum AudioChannel from, const enum AudioChannel to)
{
    for (unsigned int i = 0; i < m_channelCount; ++i)
    {
...
int AudioChannelLayout::BestMatch(const std::vector<AudioChannelLayout>& dsts, int* score) const
{
    AudioChannelLayout availableDstChannels;
   ...
   * are remappable (e.g. RC => RL+RR), do those remaps */
    ...
    for (unsigned int i = 0; i < dsts.size(); i++)
    {
        const AudioChannelLayout& dst = dsts[i];
        int okChannels = 0;
...
void AudioChannelLayout::AddMissingChannels(const AudioChannelLayout& rhs)
{
