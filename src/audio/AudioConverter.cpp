...
template<>
AudioConf AudioConverter<int16_t,float>::onProcess(const AudioConf& conf, AudioBuffer& buffer)
{
...
template<>
AudioConf AudioConverter<float,int16_t>::onProcess(const AudioConf& conf, AudioBuffer& buffer)
{
    char* to = buffer.acquire(buffer.size()/2);
    char* from = buffer.data();
...
AudioConf AudioConverter<T,U>::onProcess(const AudioConf& conf, AudioBuffer& buffer)
{
