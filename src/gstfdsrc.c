 ...
 *
 * If the #GstFd2Src:timeout property is set to a value bigger than 0, fdsrc will
 * generate an element message named <classname>&quot;GstFd2SrcTimeout&quot;</classname>
...
gst_fd_src_set_property (GObject * object, guint prop_id, const GValue * value,
    GParamSpec * pspec)
{
  GstFd2Src *src = GST_FD_SRC (object);
    ...
    if (G_UNLIKELY (retval == -1)) {
      ...
      gst_element_post_message (GST_ELEMENT_CAST (src),
          gst_message_new_element (GST_OBJECT_CAST (src),
              gst_structure_new ("GstFd2SrcTimeout",
...
{
  gboolean ret = FALSE;
  GstFd2Src *src = GST_FD_SRC (basesrc);
...
{
  GstFd2Src *src = GST_FD_SRC (handler);
